...
## Additional 3D/4D weighted HV examples

Two additional files accompany the basic acquisition module:

- `yildiz_suri_hv.py` — includable anchored hypervolume backend based on the Yildiz--Suri-style implementation provided for this project. Repository reference supplied by Michael T. M. Emmerich: <https://github.com/emmerichmtm/ImplementationOfYilidizAndSuriSubquadratic4DHypervolume>.
- `weighted_hv_kernel_examples_3d4d.py` — standalone 3D and 4D examples for product-kernel weighted hypervolume using the same aspiration--reservation desirability idea with `epsilon = 0.05`.
...
The script is standalone: it contains a small exact inclusion--exclusion hypervolume fallback for small examples. If `yildiz_suri_hv.py` is available in the same folder, it is loaded as an optional backend and compared against the fallback.
